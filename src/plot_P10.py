...
#    count += 1
for i in range(3, len(sp_state), 14):
    if (i == 3):
        P10inf = (sp_state[i-3] + sp_state[i] + sp_state[i+3] + sp_state[i+6] + sp_state[i+9] + sp_state[i+10]) / 6
    else:
...
plt.title(r'P = 10')
plt.savefig('../pics/fig_P10_w-noise.png')
plt.savefig('../pics/fig_P10_w-noise.pdf')
plt.show()
